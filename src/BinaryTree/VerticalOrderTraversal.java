...
public class VerticalOrderTraversal {
	...
	private static void verticallevelOrderTraversal(Node node) {
		...
		addToListOfMapIfNotPresent(mapList, node, 0);
		while(!stackLIFO.isEmpty()) {
			...
			if(node.leftNode != null) {
				...
				addToListOfMapIfNotPresent(mapList, node.leftNode, horizontalDistance.get(node)-1);
			}
			if(node.rightNode != null) {
				stackLIFO.add(node.rightNode);
				horizontalDistance.put(node.rightNode, horizontalDistance.get(node)+1);
				addToListOfMapIfNotPresent(mapList, node.rightNode, horizontalDistance.get(node)+1);
			}
		}
	...
	private static void addToListOfMapIfNotPresent(Map<Integer, List<Node>> mapList, Node currentNode, int distance) {
		List<Node> listOfNodes = null;
		if(mapList.containsKey(distance)) {
